...
var gameLocationMatrixX = new Array();
var gameLocationMatrixY = new Array();
var currentFoodW;
var currentFoodH;
var nextMove;
var headW;
...
window.onload = function starting() {
    ...
    for (h=0; h<28; h++) {
        gameLocationMatrixX[h] = (h + 1) * 20;
    }

    for (w=0; w<38; w++) {
        gameLocationMatrixY[w] = (w + 1) * 20;
        gameMatrix[w] = new Array();
        for (h=0; h<28; h++) {
...
function resetGame() {
    ...
    for (w=0; w<38; w++) {
        for (h=0; h<28; h++) {
            gameMatrix[w][h] = "empty";
            gameLocationMatrixX[w] = (h + 1) * 20;
            gameLocationMatrixY[h] = (w + 1) * 20;
            ...
            if (gameMatrix[w][h]>0) {
                if (gameMatrix[w][h] === "empty") {
                    ...
                    if(headW == w && headH == h) {
                        ...
                        ctx.fillRect(gameLocationMatrixY[h],gameLocationMatrixX[w],18,18);
                        ctx.fillStyle = "white";
                    } else {
                        gameMatrix[w][h] = gameMatrix[w][h] - 1;
                        ctx.fillRect(gameLocationMatrixY[h],gameLocationMatrixX[w],18,18);
